    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex gap-2">
          ...
          {/* WhatsApp direct button — opens notes modal */}
          <button
            onClick={(e) => { e.stopPropagation(); setOrderNotes(""); setOrderModalOpen(true); }}
        ...
        >
          ...
          >
            ...
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-xl font-bold mb-1" style={{ color: "#E8D5A0", fontFamily: "'IBM Plex Sans Arabic', 'Cairo', sans-serif" }}>
                  تفاصيل الطلب
                </h3>
                <p className="text-sm" style={{ color: "rgba(201,168,76,0.6)" }}>{product.name}</p>
              </div>
              <button
            ...
            {/* Product summary */}
            <div className="flex items-center gap-3 rounded-2xl p-3" style={{ background: "rgba(156,122,60,0.08)", border: "1px solid rgba(156,122,60,0.15)" }}>
              <img src={product.image} alt={product.name} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
              <div>
                <p className="font-semibold text-sm" style={{ color: "#E8D5A0" }}>{product.name}</p>
                <p className="text-sm font-bold" style={{ color: "#C9A84C" }}>{product.price}</p>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(156,122,60,0.25)", background: "rgba(255,255,255,0.04)" }}>
                  <button type="button" onClick={() => setOrderQty((q) => Math.max(1, q - 1))}
                    className="w-10 h-11 flex items-center justify-center text-lg font-bold"
                    style={{ color: "#C9A84C", background: "rgba(156,122,60,0.1)" }}>-</button>
